    ...
    if(!$retrieveRow){
      ...
      while($row = pg_fetch_array($retrieveRow)){
        ...
        while ($startDate->format('Y-m-d') <= $endDate->format('Y-m-d')) {
          ...
          $avail = "t";
          $q3 = "UPDATE service_dates SET availability = '$avail' WHERE date_avail = '$insertDate' AND serviceid = '$serviceid'";
          $updateDateStatus = pg_query($db,$q3);
          if(!$updateDateStatus){ echo "Error in updating service_dates";}
